document.addEventListener('DOMContentLoaded', () => {
    ...
    let galleryBackButton = null; // Variabile per il pulsante indietro dinamico
    ...
    // Gestisce aggiunta/rimozione bottone indietro e titolo galleria nell'header
    function switchView(viewToShow) {
        ...
        if (activeView === galleryView) {
            ...
            if (galleryTitle && mainHeaderContent && !mainHeaderContent.contains(galleryTitle)) {
                 mainHeaderContent.appendChild(galleryTitle);
    ...
    try {
        ...
        // Assicura che il bottone indietro non sia presente all'inizio
        const initialBtn = document.getElementById('gallery-back-button');
        console.log('Init: Checking for initial back button:', initialBtn); // Log if found
        if (initialBtn && mainHeaderContent) {
             try {
                 // Tentativo di rimozione più diretto
                 if (initialBtn.parentNode === mainHeaderContent) { // Verifica se è figlio diretto
                      mainHeaderContent.removeChild(initialBtn);
                      console.log("Pulsante Indietro rimosso dall'header (Init - Success)");
                 } else {
                      console.log("Pulsante Indietro trovato, ma non nell'header? Parent:", initialBtn.parentNode);
                 }
             } catch (error) {
                 console.error("Errore durante la rimozione iniziale del bottone:", error);
             }
         } else if (!initialBtn) {
              console.log("Init: Pulsante Indietro non trovato nel DOM.");
         }
        // Assicura che il titolo sia visibile all'inizio
        if(mainTitle) mainTitle.style.display = '';
