document.addEventListener('DOMContentLoaded', () => {
    ...
    const foldersData = [
        // ... (dati cartelle completi come prima) ...
        {
            ...
            id: "cartella5",
            photos: [ { image: "images/cartella5/1.webp"}, { image: "images/cartella5/2.webp"}, { image: "images/cartella5/3.webp"}, { image: "images/cartella5/4.webp"}, { image: "images/cartella5/5.webp"}, { image: "images/cartella5/6.webp"}, { image: "images/cartella5/7.webp"}, ]
        },
    ];
    ...
    const lightboxImg = lightbox?.querySelector('.lightbox-content'); // Seleziona l'immagine
    ...
    const essentialElements = { folderView, galleryView, folderGrid, photoGrid, galleryTitle, backButton, mainElement, currentYearSpan, lightbox, lightboxImg, lightboxCaption, lightboxAudio, lightboxPrev, lightboxNext, lightboxContainer };
    ...
    function switchView(viewToShow) { /* ... (invariata) ... */ if (!viewToShow || viewToShow === activeView) return; const oldView = activeView; activeView = viewToShow; if (mainElement) { if (activeView === galleryView) { mainElement.classList.add('gallery-active'); removeFolderTiltInteraction(); } else { mainElement.classList.remove('gallery-active'); initFolderTiltInteraction(); } } if (oldView) { oldView.classList.remove('active'); setTimeout(() => { if (!oldView.classList.contains('active')) oldView.style.display = 'none'; }, 500); } viewToShow.style.display = 'block'; requestAnimationFrame(() => { viewToShow.classList.add('active'); updateContentAreaHeight(); }); window.scrollTo({ top: 0, behavior: 'smooth' }); }
    ...
    function showGallery(folderIndex) { /* ... (invariata) ... */ try { const folder = foldersData[folderIndex]; if (!folder) return; currentFolderIndex = folderIndex; galleryTitle.textContent = folder.name || `Galleria ${folderIndex + 1}`; photoGrid.innerHTML = ''; currentPhotos = folder.photos || []; if (currentPhotos.length > 0) { currentPhotos.forEach((photoData, photoIndex) => { if (!photoData || !photoData.image) return; const cont = document.createElement('div'); cont.className = 'photo-item-container'; cont.setAttribute('role', 'button'); cont.tabIndex = 0; const img = document.createElement('img'); img.className = 'photo-item'; img.src = photoData.image; img.alt = photoData.caption || `Immagine ${photoIndex + 1} da ${folder.name || ''}`; img.loading = 'lazy'; img.onerror = () => { img.alt = `Errore caricamento: ${photoData.image}`; cont.style.border = '2px dashed #ff6b6b'; }; const handler = () => openLightbox(photoIndex); cont.addEventListener('click', handler); cont.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handler(); }}); cont.appendChild(img); photoGrid.appendChild(cont); }); } else { photoGrid.innerHTML = '<p class="loading-message">Nessuna immagine in questa sezione.</p>'; } switchView(galleryView); } catch (error) { console.error("Errore showGallery:", error); photoGrid.innerHTML = '<p class="loading-message" style="color:red;">Errore galleria.</p>'; } }
    ...
    // --- Impostazione Event Listener (Esistenti + Nuovo) ---
    console.log("Impostazione event listeners..."); // Modificato log
    try {
        backButton.addEventListener('click', () => { closeLightbox(); switchView(folderView); });

        // Listener per chiudere cliccando sullo sfondo
        lightbox.addEventListener('click', (e) => {
            const clickedOnInteractive = e.target.closest('.lightbox-container, .lightbox-nav, .lightbox-audio-controls');
            if (!clickedOnInteractive && e.target === lightbox) {
                 closeLightbox();
            }
        });

        // --- NUOVO: Listener per chiudere cliccando sull'immagine ---
        if (lightboxImg) {
        ...
        if (lightboxImg) { /* ... (gestore img invariato, MA ora ha anche il click listener) ... */ lightboxImg.onerror = () => { console.error("--- ERRORE IMG LIGHTBOX ---", lightboxImg.src); if (lightboxCaption) lightboxCaption.textContent = "Errore caricamento immagine."; lightboxImg.alt = "Errore caricamento immagine"; }; }
